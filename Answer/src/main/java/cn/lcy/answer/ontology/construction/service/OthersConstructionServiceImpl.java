...
public class OthersConstructionServiceImpl implements ConstructionServiceI {
...
private ConstructionDAOI constructionDAO = new ConstructionDAOImpl();
	...
	public boolean construction(BaikePage baikePage) throws Exception {
		...
		othersIndividual = this.queryIndividual(individualName, polysemantExplain, url, true, OntologyClassEnum.OTHERS);
		...
		if(picSrc != null) {
			...
			PictureDownloader.picDownload(picSrc, newPicName, ConstructionBaseDAOImpl.PIC_SAVE_PATH + "//" + OntologyClassEnum.OTHERS.getName() );
			constructionDAO.addDataProperty(othersIndividual, "picSrc", newPicName);
		}
	...
	public Individual queryIndividual(String individualName, String polysemantExplain, String url, boolean isAliases, OntologyClassEnum parentClass) {
		...
		if(othersIndividual == null) {
			...
			OntClass othersClass = constructionDAO.getOntClass(parentClass.getName());		// 获取电影类型
			othersIndividual = constructionDAO.createIndividual(movieIndividualUUID, othersClass);
		}
		constructionDAO.addComment(othersIndividual, individualName); // 创建comment
